...
public class ShortcodeGenerator {
	...
	private final static char allowedChars_64[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-"
			.toCharArray();
	private final static char allowedChars_62[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
			.toCharArray();
	...
	public String generateRandomShortcode(int length) {
		...
		for (int i = 0; i < length; i++) {
			int randomIndex = ThreadLocalRandom.current().nextInt(0, allowedChars_64.length);
			shortcode.append(allowedChars_64[randomIndex]);
		}
	...
	public String generateShortcodeUsingId(long id) {
		...
		while (id > 0) {
			shortcode.append(allowedChars_62[(int) (id % allowedChars_62.length)]);
			id = id / allowedChars_62.length;
		}
	...
	public long convertShortURLtoID(String shortcode) {
		...
		for (int i = 0; i < shortcode.length(); i++) {
			if ('a' <= shortcode.charAt(i) && shortcode.charAt(i) <= 'z')
				id = id * allowedChars_62.length + shortcode.charAt(i) - 'a';
			if ('A' <= shortcode.charAt(i) && shortcode.charAt(i) <= 'Z')
				id = id * allowedChars_62.length + shortcode.charAt(i) - 'A' + 26;
			if ('0' <= shortcode.charAt(i) && shortcode.charAt(i) <= '9')
				id = id * allowedChars_62.length + shortcode.charAt(i) - '0' + 52;
		}
		return id;
	}
